...
function updateRow(type, number) {
    number = Number.parseFloat(number).toPrecision(2);
    $(`#myTable tr.${type} td`).text(`${number}%`);
}

updateRow("adept", totalCount>0? window.counts.adept/totalCount : window.counts.adept);
updateRow("detective", totalCount>0? window.counts.detective/totalCount : window.counts.detective);
updateRow("slowDetective", totalCount>0? window.counts.slowDetective/totalCount : window.counts.slowDetective);
updateRow("failed", totalCount>0? window.counts.failed/totalCount : window.counts.failed);
